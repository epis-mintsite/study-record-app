...
const studentSections: Section[] = [
  ...
  },
  {
    ...
    steps: [
      ...
      {
        title: '学習内容を手入力する',
        desc: 'ナビの「記録する」を開きます。開始・終了時刻・科目・メモを入力して「保存する」を押すと週間表に反映されます。「＋ 追加」で複数のコマを一度に登録できます。',
        sub: [
          '開始・終了時刻は時計アイコンから選択できます',
          '「＋ 追加」ボタンで学習コマを増やせます',
        ],
      },
      {
  ...
  },
  {
    ...
    steps: [
      {
        title: '過去問演習の記録を登録する',
        desc: 'ナビの「過去問」を開き「＋ 追加」ボタンを押します。志望校名・入試区分（一般／帰国）・年度・実施日・科目別の得点を入力して保存します。',
        sub: [
          '高校名は過去に入力した候補から選ぶこともできます',
          '得点は科目ごとに複数登録できます（満点は任意）',
        ],
      },
      {
        title: '合格最低点と見比べる',
        desc: '登録済みの学校・年度に合格最低点（管理者が登録した参考値）が設定されている場合、得点の横に自動で表示されます。得点が上回っていれば緑、下回っていればオレンジで色分けされます。',
        note: '合格最低点は学校発表の正式な数値とは異なる場合がある参考値です。',
      },
      {
        title: '得点率の推移を確認する',
        desc: '学校名のタブで特定の志望校を選ぶと、科目別の得点率（％）の推移グラフと、直近年度の合格最低点ラインが表示されます。',
      },
    ],
  },
  {
...
const parentSections: Section[] = [
  ...
  },
  {
    ...
    steps: [
      ...
      {
        title: '過去問演習を確認する',
        desc: '「過去問」タブで、お子さまが記録した志望校の過去問演習を確認できます。学校の合格最低点（参考値）が登録されていれば、得点との比較も表示されます。',
      },
      {
        title: '振返りを確認する',
...
const adminSections: Section[] = [
  ...
  },
  {
    ...
    steps: [
      ...
      {
        title: '生徒の学習状況を閲覧する',
        desc: 'ユーザー一覧の生徒名（青いリンク）をタップすると、その生徒の学習記録ページに移動できます。週間表・成績・過去問演習・振返りを管理者として閲覧できます。',
        badge: '管理者閲覧（緑バッジ）',
        badgeColor: '#ECFDF5',
        badgeTextColor: '#065F46',
      },
      {
        title: '合格最低点（参考）を登録する',
        desc: 'ダッシュボードの「合格最低点（参考）の管理」から管理画面を開き、学校名・入試区分・年度・科目ごとに合格最低点を登録します。生徒・保護者の過去問演習画面に自動で反映されます。',
        sub: [
          '同じ学校・区分・年度・科目で登録すると上書き更新されます',
          '満点は任意（未入力でも登録可能）',
        ],
        badge: '管理者専用',
        badgeColor: '#ECFDF5',
  ...
  },
  {
    ...
    steps: [
      ...
      {
        title: '週次学習トレンド（毎週月曜のみ）',
        desc: '毎週月曜7時の日報には、先週分の学習傾向をClaudeが分析した「週次学習トレンド」が自動で追記されます。科目別に先週比で大きく増減した生徒や、全体の学習量の変化を自然文で要約します。',
        sub: [
          '数値は集計データのみを根拠に生成されます（AIが数値を創作することはありません）',
          '生成に失敗した場合も、通常の日報部分は問題なく送信されます',
        ],
        badge: 'AI自動生成',
        badgeColor: '#F5F3FF',
        badgeTextColor: '#6D28D9',
...
export default function GuidePage() {
  ...
  const config: Record<Tab, {
    ...
    student: {
      ...
      features: ['📅 週間学習グリッド', '✏️ 手入力記録', '🏷️ カスタム科目', '📊 成績登録', '📝 過去問演習', '💡 AI振返り支援', '🏆 学習ランキング', '🔔 朝7時の通知'],
    },
    parent: {
      label: '保護者',
      emoji: '👨‍👧',
      ...
      features: ['📅 週間表の閲覧', '📊 成績の確認', '📝 過去問演習の確認', '💡 振返りの確認', '📈 月間学習チャート', '🏆 ランキング閲覧', '🔒 閲覧専用'],
    },
    admin: {
      label: '管理者',
      emoji: '🛠️',
      ...
      features: ['👥 ユーザー一覧', '📋 生徒ページ閲覧', '🎓 合格最低点管理', '📨 Slack日報（自動・手動）', '📈 週次AIトレンド', '🏆 ランキング閲覧', '🔔 朝7時の通知', '👤 デモアカウント発行'],
    },
  };
